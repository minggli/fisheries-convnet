...
def batch_retrieve(func, iterable):
    """processing through iterable (e.g. list)"""
    from multiprocessing import Pool

    with Pool(4) as p:
        p.starmap(func, zip(iterable, CV_SAMPLE_PATH + 'neg'))


def retrieve_image(image_url, path):
    """download single image and save in path"""
    r = requests.get(image_url, allow_redirects=True, timeout=5, stream=True)
    code = r.status_code
...
sample_pos = generate_sample_skeleton(synset_ids_pos, sample_size=50)
sample_neg = generate_sample_skeleton(synset_ids_neg, sample_size=50)
print(len(sample_pos))
print(len(sample_neg))
# retrieve_image(image_url=sample_neg[5], path=CV_SAMPLE_PATH + 'neg')
batch_retrieve(func=retrieve_image, iterable=sample_neg)
