...
def predict(sess, path, test):
    ...
    probs = sess.run(tf.nn.softmax(read_out), feed_dict={x: test, keep_prob: 1.0})
    return probs


def submit(probs, path):
    ...
    df = pd.DataFrame(data=probs, columns=template.columns, dtype=np.float32, index=template.index)
    df.to_csv(path + 'submission_{0}.csv'.format(now), encoding='utf8', header=True, index=True)
